...
@pages.route("/")
def index():
    movie_data = current_app.db.titles.find({})
...
def add_movie():
    ...
    if form.validate_on_submit():
        ...
        current_app.db.titles.insert_one(asdict(movie))
...
def movie(_id: str):
    movie = Movie(**current_app.db.titles.find_one({"_id": _id}))
...
def edit_movie(_id: str):
    ...
    if form.validate_on_submit():
        ...
        current_app.db.titles.update_one({"_id": movie._id}, {"$set": asdict(movie)})
        return redirect(url_for(".movie", _id=movie._id))
...
def watch_today(_id):
    current_app.db.titles.update_one(
...
def rate_movie(_id):
    ...
    current_app.db.titles.update_one({"_id": _id}, {"$set": {"rating": rating}})
    return redirect(url_for(".movie", _id=_id))
